/*
  ...
  Version 1.9.10: Added support for floats & Strings for Uno (without the CC3000
  chip)
  ...
  Version 1.9.4: Bug fixes & added support for configuring analog pints as
  digital outputs
  Version 1.9.3: Added description of available variables for the /id and /
  routes
  Version 1.9.2: Added compatibility with the Arduino WiFi library
  Version 1.9.1: Added compatibility with CORS
  Version 1.9: New speedup of the library (answers 2x faster in HTTP compared to
  version 1.8)
  Version 1.8: Speedup of the library (answers 2.5x faster with the CC3000 WiFi
  chip)
  ...
  Version 1.7.1: Added possibility to change number of exposed variables &
  functions
...
// Which board?
    ...
    defined(CORE_WILDFIRE) || defined(ESP8266)
...
#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__) ||              \
    defined(CORE_WILDFIRE) || defined(ESP8266) || !defined(ADAFRUIT_CC3000_H)
...
public:
  ...
  // Get topic
  char *get_topic() { return out_topic; }
  ...
  void publish(PubSubClient &client, String eventName, T data) {
    ...
    // Build message
    String message = "{\"client_id\": \"" + String(id) +
...
// Handle request for the Arduino WiFi shield
  ...
  // Process callback
  void handle_callback(PubSubClient &client, char *topic, byte *payload,
                       unsigned int length) {
    ...
    if (strlen(answer) < max_message_size) {
      ...
      for (uint8_t i = 0; i < max_iteration; i++) {
        ...
        memcpy(intermediate_buffer, buffer + i * max_message_size,
               max_message_size);
...
#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__) ||              \
    defined(ESP8266) || defined(CORE_WILDFIRE) || !defined(ADAFRUIT_CC3000_H)
...
// String variable selected (Mega & ESP8266 only)
  ...
  virtual void root_answer() {
...
#if defined(ADAFRUIT_CC3000_H) || defined(ESP8266) || defined(ethernet_h) ||   \
    defined(WiFi_h)
#if !defined(PubSubClient_h)
    if (command != 'u') {
      addToBuffer(F("HTTP/1.1 200 OK\r\nAccess-Control-Allow-Origin: "
    ...
    defined(ESP8266) || defined(CORE_WILDFIRE) || !defined(ADAFRUIT_CC3000_H)
      ...
      // Int variables
      if (variables_index == 0 && string_variables_index == 0 &&
          float_variables_index == 0) {
        addToBuffer(F(" }, "));
      } else {
...
// String variables (Mega & ESP only, or without CC3000)
  ...
  // Set device name
  void set_name(char *device_name) { strcpy(name, device_name); }
  ...
  void removeLastBufferChar() { index = index - 1; }
...
// Memory debug
#if defined(ESP8266)
  void initFreeMemory() { freeMemory = ESP.getFreeHeap(); }
#endif

#if defined(PubSubClient_h)
  void setMQTTServer(char *new_mqtt_server) { mqtt_server = new_mqtt_server; }
#endif
